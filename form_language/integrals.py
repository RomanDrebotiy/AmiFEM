...
class Integral:
    def __init__(self, measure_map: Dict[Measure, Expression] | None = None):
        ...
        self.quad_map: Dict[Measure, Callable] = {
            Measure.DX: self.__quad_space # only supporting space integrals for now
        }
    ...
    def __quad_space(self, f, v0, v1, v2):
        points = [
            (1 / 2, 1 / 2, 0),
            (1 / 2, 0, 1 / 2),
            (0, 1 / 2, 1 / 2)
    ...
    def eval(self, v0, v1, v2, trial_func: Callable[[float, float], np.ndarray] | None, test_func: Callable[[float, float], np.ndarray] | None) -> float:
        res = 0
